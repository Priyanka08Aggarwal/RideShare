...
const ConfirmBooking = (props) => {
  ...
  function RideAvailability(props) {
    if (!(props.pickup in assignDriver[props.category]))
      return (
        <h2>
          No available drivers under {location.state.category} Category for
          pickup Location '{location.state.pick}'
        </h2>
      );
    else
      return (
        <>
          ...
          <div className="ride_container">
            <div className="ride_container_inner">
              ...
              <div>
                ...
                <h6>
                  {
                    assignDriver[location.state.category][location.state.pick][
                      "tripId"
                    ]
                  }
                </h6>
          ...
          <button
            id="cancelButton"
            type="submit"
            onClick={() => history("/rideOptions/cancelBooking")}
          >
            {" "}
            Cancel Ride{" "}
          </button>
  ...
  let parms = {
    ...
    epoch: Date.now(),
    tripId:
      assignDriver[location.state.category][location.state.pick]["tripId"],
    ...
    return (
      <div>
        ...
        <button
          type="submit"
          onClick={() =>
            history("/rideOptions/rateBooking", {
              state: {
                tripId:
                  assignDriver[location.state.category][location.state.pick][
                    "tripId"
